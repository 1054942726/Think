  ...
  methods: {
    ...
    repaintNode(parentIndex, edit) {
      ...
      subList.forEach((sub, index) => {
        ...
        // }
        // else
        if (index == 0 && !this.nodeList[sub].top) {
          console.log("3rank");
          this.nodeList[sub].top =
            parseFloat(parentNode.top) -
          ...
          this.nodeList[sub].top =
            ...
            "px";
          console.log("===================");
        } else if (this.nodeList[sub].top) {
          this.nodeList[sub].top =
            parseFloat(this.nodeList[sub].top) - (25 + 41.96) / 2 + "px";
        }
        //判断是否有节点top小于0
      ...
      let cha =
        this.getListHeight(subList) / 2 -
        (parseFloat(parentNode.top) -
          parseFloat(this.nodeList[subList[0]].top));
      console.log(
        "lsitHeiht:",
        this.getListHeight(subList),
        "parentTop:",
        parentNode.top,
        "0Top:",
        this.nodeList[subList[0]].top
      );
      subList.forEach(sub => {
        this.nodeList[sub].top =
          parseFloat(this.nodeList[sub].top) - cha + "px";
    ...
    reapintParent(parentIndex) {
      ...
      if (parentNode.rank != 1) {
        ...
        parentList.forEach((node, index) => {
          ...
          if (index < thisIndex) {
            if (this.nodeList[node].rank == 2) {
              this.nodeList[node].top =
                parseFloat(this.nodeList[node].top) - 45 + "px";
            } else if (
              parentList.length == 2 &&
              this.nodeList[node].subNode.length == 0 &&
              this.nodeList[this.selectIndex].subNode.length == 2
            ) {
              this.nodeList[node].top =
                parseFloat(this.nodeList[node].top) - 1 + "px";
            } else {
              this.nodeList[node].top =
                parseFloat(this.nodeList[node].top) - 40 + "px";
            }
    ...
    getListHeight(list) {
      //传入一个节点列表，得到该列表第一个top到最后一个top的距离
      if (list.length >= 2) {
        return (
          parseFloat(this.nodeList[list[list.length - 1]].top) -
          parseFloat(this.nodeList[list[0]].top) +
          42
        );
      } else {
        return 42;
